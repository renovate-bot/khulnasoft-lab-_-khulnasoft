...
const debug = createDebug('@khulnasoft/fun:install-node');
...
export async function installNode(
	...
	if (platform === 'win32') {
		...
		await pipe(res.body, createWriteStream(zipPath));
		...
		await pipe(res.body, createGunzip(), extract({ strip: 1, C: dest }));
	}
}
