  ...
  if (wrapper) {
    ...
    //   });
      ...
      (wrapper.arguments[0].body.body.length === 1 ||
        (wrapper.arguments[0].body.body.length === 2 &&
          ...
          wrapper.arguments[0].body.body[0].declarations.every(
            decl => decl.init === null && decl.id.type === 'Identifier'
          ))) &&
      wrapper.arguments[0].body.body[wrapper.arguments[0].body.body.length - 1]
        .type === 'ReturnStatement' &&
      (browserifyReturn = wrapper.arguments[0].body.body[
        wrapper.arguments[0].body.body.length - 1
      ]! as ReturnStatement) &&
      browserifyReturn.argument?.type === 'CallExpression' &&
      browserifyReturn.argument.arguments.length &&
      browserifyReturn.argument.arguments.every(
        arg => arg && arg.type === 'Literal' && typeof arg.value === 'number'
      ) &&
    ...
    //   }
      ...
      if (
        ...
        let callSite;
        if (
          statement.consequent.body[0].type === 'VariableDeclaration' &&
    ...
    //
      ...
      (wrapper.arguments.length === 0 &&
        ...
        wrapper.callee.body.body[3].expression.right.type ===
          'ObjectExpression' &&
        wrapper.callee.body.body[3].expression.right.properties.every(
          prop =>
            prop &&
            prop.type === 'Property' &&
            !prop.computed &&
      ...
      let modules: [string, FunctionExpression][];
      if (webpackModuleObj.type === 'ArrayExpression')
        modules = (
          webpackModuleObj.elements.filter(
            el => el?.type === 'FunctionExpression'
          ) as FunctionExpression[]
        ).map((el, i) => [String(i), el]);
      // Structure already checked in conditional above
      else
        modules = webpackModuleObj.properties.map(
          prop =>
            [
              String(((prop as Property).key as Literal).value),
              (prop as Property).value as FunctionExpression,
            ] as [string, FunctionExpression]
        );
      for (const [k, m] of modules) {
        const statement =
          m.body.body.length === 1
            ? m.body.body[0]
          ...
          walk(m.body as unknown as ESNode, {
            enter(_node, _maybeParent) {
              ...
              if (
                ...
                const externalId = externalMap.get(
